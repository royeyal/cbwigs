 ...
 * Creates a trailing effect of images that follow the cursor movement
 */
...
export function initImageTrail(config = {}) {
  ...
  // Early return if wrapper doesn't exist or viewport is too narrow
  ...
  function fadeOutTrailImage(trailImage) {
    ...
    window.gsap.to(trailImage, {
      opacity: 0,
      scale: 0.2,
      duration: 0.8,
      ease: 'expo.out',
      onComplete: () => {
        window.gsap.set(trailImage, { autoAlpha: 0 });
      }
    });
  }
